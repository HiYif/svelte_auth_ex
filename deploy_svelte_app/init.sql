...
CREATE OR REPLACE PROCEDURE public.reset_password_v2(
	input_id integer,
	input_password text)
LANGUAGE plpgsql
AS $$
BEGIN
    -- Your SQL statements and transactions here
    UPDATE users SET password = crypt(input_password, gen_salt('bf', 8)) WHERE id = input_id;
    COMMIT;
END;
$$;
...
ALTER PROCEDURE public.reset_password_v2(integer, text) OWNER TO auth;
